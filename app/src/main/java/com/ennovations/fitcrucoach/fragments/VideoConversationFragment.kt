...
import com.ennovations.fitcrucoach.quickbox.adapters.OpponentsFromCallAdapter
...
class VideoConversationFragment : BaseConversationFragment(), Serializable,
    ...
    private fun updateNameForOpponent(userId: Int, newUserName: String?) {
        if (isPeerToPeerCall) {
            actionBar.subtitle = getString(R.string.opponent, newUserName ?: "dummy")
        } else {
            val holder = findHolder(userId)
            if (holder == null) {
            ...
            Log.d("UPDATE_USERS", "holder != null")
            holder.setUserName(newUserName ?: "dummy")
        }
    }
